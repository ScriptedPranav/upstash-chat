...
export default function Home() {
  ...
  useEffect(() => {
    ...
    socket?.on(NEW_MESSAGE_CHANNEL, (message: Message) => {
      ...
      // Scroll to bottom after we have received the message
      setTimeout(() => {
        scrollToBottom();
      },0)
    });

    // Get the total connection count from socket
  ...
  return (
    <main className="flex flex-col p-4 w-full max-w-3xl m-auto">
      <h1 className="text-4xl font-bold text-center">Chat({connectionCount})</h1>
      <ol
